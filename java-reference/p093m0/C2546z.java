package p093m0;

import p022c4.AbstractC0398h;
import p034e0.C0619n;
import p085l0.AbstractC2399q;
import p085l0.C2350d2;
import p085l0.InterfaceC2343c;
import p145t0.C3179j;

/* compiled from: r8-map-id-091eedbcc63bb3784ce17035a64051c23f885688c02fb159d88a8d507f170047 */
/* renamed from: m0.z */
/* loaded from: classes.dex */
public final class C2546z extends AbstractC0398h {

    /* renamed from: d */
    public static final C2546z f13914d = new AbstractC0398h(0, 0, 3);

    @Override // p022c4.AbstractC0398h
    /* renamed from: a */
    public final void mo873a(C0619n c0619n, InterfaceC2343c interfaceC2343c, C2350d2 c2350d2, C3179j c3179j, InterfaceC2528i0 interfaceC2528i0) {
        if (c2350d2.f13372n != 0) {
            AbstractC2399q.m3900c("Cannot reset when inserting");
        }
        c2350d2.m3750F();
        c2350d2.f13378t = 0;
        c2350d2.f13379u = c2350d2.m3778o() - c2350d2.f13366h;
        c2350d2.f13367i = 0;
        c2350d2.f13368j = 0;
        c2350d2.f13373o = 0;
    }
}
